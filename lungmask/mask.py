from lungmask.lungmask.utils import Action
import numpy as np
import torch
...
#Adding Aggregate Pattern for course purpose(Software design)
# stores urls and number of classes of the models
class Apply_Model:

    #I Add a new class Apply_Model, then I put three method into it. （apply, get_model, apply_fused）
    # In my perspective, I think apply and apply fused are in the same domain, their purpose is apply the model with different way.
    # I add them into Apply_Model class, and when main.py want to call these methods, it must go through the class Apply_Model
    # It need to create a instance of Apply_Model
    def __init__(self, image, modeltype, modelname):
        self.iamge = image
    ...
    def apply(image, model=None, force_cpu=False, batch_size=20, volume_postprocessing=True, noHU=False):
        ...
        if model is None:
            model = Apply_Model.get_model('unet', 'R231')
        ...
        #I want to let Action Class in the utils to be a instant, then use the preprocess will just call the instance
        if not noHU:
            tvolslices, xnew_box = Action.preprocess(inimg_raw, resolution=[256, 256])
        ...
        # neighbors
        #I want to let Action Class in the utils to be a instant, then use the postprocess will just call the instance
        if volume_postprocessing:
            outmask = Action.postrocessing(timage_res)
        else:
            outmask = timage_res
        ...
        if noHU:
            ...
            outmask = np.asarray(
                [utils.Action.reshape_mask(outmask[i], xnew_box[i], inimg_raw.shape[1:]) for i in range(outmask.shape[0])],
                dtype=np.uint8)
    ...
    def apply_fused(image, basemodel = 'LTRCLobes', fillmodel = 'R231', force_cpu=False, batch_size=20, volume_postprocessing=True, noHU=False):
        ...
        mdl_l = Apply_Model.get_model('unet',basemodel)
        logging.info("Apply: %s" % basemodel)
        res_l = Apply_Model.apply(image, mdl_l, force_cpu=force_cpu, batch_size=batch_size,  volume_postprocessing=volume_postprocessing, noHU=noHU)
        logging.info("Apply: %s" % fillmodel)
        res_r = Apply_Model.apply(image, mdl_r, force_cpu=force_cpu, batch_size=batch_size,  volume_postprocessing=volume_postprocessing, noHU=noHU)
        ...
        return Action.postrocessing(res_l, spare=[spare_value])
